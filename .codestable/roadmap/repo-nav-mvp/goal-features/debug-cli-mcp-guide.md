...
- DoD contract results?`.codestable/features/2026-07-10-debug-cli-mcp-guide/debug-cli-mcp-guide-dod-contract-results.json`

## 2. Delivery And Core Path

- ????????? debug CLI???? MCP/API docs ? MVP aggregate verification?
- ????????? CLI locate/probe/golden?docs MCP snippets ? full aggregate command?
- Roadmap contract?design frontmatter ?? `repo-nav-mvp` / `debug-cli-mcp-guide`???????????? interface?status/reason?ordering?failure mode ????????
...
- Checklist steps ??? `pending` ??? `done`?acceptance ?? checks?C1, C2, C3, C4, C5, C6, C7, C8, C9, C10, C11, C12, C13???? `passed`?
...
2. **Review**??? `cs-code-review` ????? Task agent??? current diff?evidence pack?gate results??? `debug-cli-mcp-guide-review.md`?
3. **QA**??? `cs-feat-qa`??? design/checklist/review/evidence??? `debug-cli-mcp-guide-qa.md` ??? command logs?
4. **Acceptance**??? `cs-feat-accept`??? passed review/QA?evidence/DoD/gates??? `debug-cli-mcp-guide-acceptance.md` ??? checks/items?
