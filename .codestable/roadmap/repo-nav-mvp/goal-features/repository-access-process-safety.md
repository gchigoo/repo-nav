...
- 一句话交付物：建立 filesystem/process 安全 seams 与跨平台清理。
- 核心运行路径：无独立用户路径；以真实 filesystem/helper-process integration 为替代证据。
...
2. Review：加载 `cs-code-review` 并使用独立 Task agent，输出 `repository-access-process-safety-review.md`。
3. QA：加载 `cs-feat-qa`，输出 `repository-access-process-safety-qa.md` 与真实运行证据。
4. Acceptance：加载 `cs-feat-accept`，输出 `repository-access-process-safety-acceptance.md` 并更新 checks/items。
