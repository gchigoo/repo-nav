...
- 一句话交付物：建立工程、schema v1、DI skeleton 与 unit/Golden/MCP runner 基线。
- 核心运行路径：无独立用户路径；以 build/typecheck、contract/DI 和 runner smoke 为替代证据。
...
2. Review：加载 `cs-code-review` 并使用独立 Task agent，输出 `repository-evidence-foundation-review.md`。
3. QA：加载 `cs-feat-qa`，输出 `repository-evidence-foundation-qa.md` 与真实运行证据。
4. Acceptance：加载 `cs-feat-accept`，输出 `repository-evidence-foundation-acceptance.md` 并更新 checks/items。
