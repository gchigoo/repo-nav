...
上面的 machine-readable 区块保留 MVP 验收契约。当前 `1.1.0` hardening checkpoint 还要求 `npm run lint`、`npm run format:check`、`npm run test:platform`、package smoke/closure、security audit、SBOM 与 fixture benchmark。真实 CodeGraph 测试位于独立的 `npm run test:integration:codegraph` surface，不属于普通 unit 前置条件。最新 CI 覆盖与后续任务见 [`docs/project-status.md`](../project-status.md)。
