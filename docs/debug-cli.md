...
当前文档对应 `repo-nav@2.0.0`。Debug CLI 是本地诊断入口，不是第二套业务语义。`locate` 复用生产 Evidence Service（schema `2.0`），`probe` 只报告基础设施健康。正式 JSON 写 stdout，安全诊断写 stderr。Public Golden 命令已移除；源码仓使用 `npm run test:golden`。
