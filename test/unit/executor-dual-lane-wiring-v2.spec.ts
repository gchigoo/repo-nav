...
class CapturingBackend implements RepositorySearchBackend {
  ...
  public async search(
    ...
    const hits = Array.from({ length: Math.min(12, request.maxHits) }, (_, index) =>
      Object.freeze({
...
describe.runIf(selected)(
  'F3-DISCOVERY-001 executor-dual-lane-wiring',
  () => {
    it('consumes expandedMaxHits=800 via shared search and runs scope fold + legacy reservation', async () => {
      expect(
        resolveSharedSearchMaxHitsV2(40, DISCOVERY_RESERVATION_CAP_V2),
      ).toBe(DISCOVERY_RESERVATION_CAP_V2);
      ...
      expect(
        deriveLaneBackendResultV2(lanes.expanded, 3).hits,
      ).toHaveLength(3);
