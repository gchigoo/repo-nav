...
function outputText(result: SafeProcessResult, stream: 'stdout' | 'stderr'): string {
...
describe.runIf(isSelected(contractIdentity))('safe process contract', () => {
  it('preserves special argv boundaries without a shell', async () => {
    ...
    try {
      ...
      expect(JSON.parse(outputText(result, 'stdout')) as unknown).toEqual(expected);
...
describe.runIf(isSelected(isolationIdentity))('safe process output isolation', () => {
  it.each([
    ['stdout', 'stdout-limit'],
    ['stderr', 'stderr-limit'],
